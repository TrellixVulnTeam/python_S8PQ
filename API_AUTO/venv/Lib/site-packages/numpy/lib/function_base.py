...
def select(condlist, choicelist, default=0):
    ...
    if len(condlist) != len(choicelist):
        raise ValueError(
            'list of testCases must be same length as list of conditions')
...
def diff(a, n=1, axis=-1, prepend=np._NoValue, append=np._NoValue):
    ...
    diff : ndarray
        ...
        `a` in most testCases. A notable exception is `datetime64`, which
...
def meshgrid(*xi, copy=True, sparse=False, indexing='xy'):
    ...
    .. versionchanged:: 1.9
       1-D and 0-D testCases are allowed.
...
def insert(arr, obj, values, axis=None):
    ...
    if indices.size == 1:
        ...
        # There are some object array corner testCases here, but we cannot avoid
        # that:
